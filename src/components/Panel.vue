<template>
  <router-link :to="station" tag="div" class="panel flex mt-[24px]">
    ...
    <svg class="shape pt-[inherit] opacity-80" viewBox="-10.929 -389.426 226.93 154" :style="{ fill: `url(#${color})` }">
      <defs>
        <gradient-color :color="color" :id="color" />
      </defs>
    ...
    <div class="flex flex-row m-h-full w-full justify-between">
      ...
      <div class="flex flex-col justify-between">
        <i-filed :number="entered" />
        <i-filed :number="exited" />
      </div>
    </div>
  </router-link>
</template>
...
import iFiled from "./panel/InformationField.vue";
...
const color = Object.keys(stations).find(key => stations[key].includes(props.station))
